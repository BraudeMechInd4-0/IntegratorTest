...
function [satrec, rx,ry,rz, vx,vy,vz, am] = sgp4cheb(satrec,tsince,consts);
   ...
   %fixed for parallel runs.
   tumin = consts(1);
   mu = consts(2);
   radiusearthkm=consts(3);
   xke=consts(4);
   j2 = consts(5);
   j3 = consts(6);
   j4 =consts(7);
...
%       fprintf(1,'# error pl %f\n', pl);
  ...
  % else
       ...
       rx = (mrt * ux)* radiusearthkm;
       ry = (mrt * uy)* radiusearthkm;
       rz = (mrt * uz)* radiusearthkm;
       %r = [ux uy uz]*mrt*radiusearthkm;
       vx = (mvt * ux + rvdot * vx) * vkmpersec;
       vy = (mvt * uy + rvdot * vy) * vkmpersec;
       vz = (mvt * uz + rvdot * vz) * vkmpersec;
       %v = (mvt*[ux uy uz]  + [vx vy vz] * rvdot)*vkmpersec;
       am =  am * radiusearthkm;
  % end % // if pl > 0
