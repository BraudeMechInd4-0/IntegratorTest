...
%    Denenberg   - add if clause                                 23 jul 2013
%
...
function [rteme,vteme,ateme] = eci2teme  ( reci,veci,aeci,ttt,order,eqeterms,opt );
        ...
        [~,~,~,~,~,nutteme] = truemean(ttt,order,eqeterms,opt); %fixed by E.D added ~, to get the matrix rather than other values

        rteme = nutteme'*prec'*reci;

        if nargout > 1                      %edited by D.E. - added if to allow the transformation of r, r,v and r,v,a
